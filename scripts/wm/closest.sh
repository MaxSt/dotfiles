...
case $1 in
    h|a|east|left)  ~/.scripts/wm/focus.sh $(next_east)  2>/dev/null ;;
    j|s|south|down) ~/.scripts/wm/focus.sh $(next_south) 2>/dev/null ;;
    k|w|north|up)   ~/.scripts/wm/focus.sh $(next_north) 2>/dev/null ;;
    l|d|west|right) ~/.scripts/wm/focus.sh $(next_west)  2>/dev/null ;;
esac
